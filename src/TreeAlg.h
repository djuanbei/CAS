...
Node *generateTree(NodeManager &node_manager, int node_num, int degree,
                   int depth);

template <typename T> struct RawNode {
  int parent{-1};
  T value;
};
...
void priorityFirst(const Node *node, nodeCheck_t is_ok, weight_fun_t w_fun,
                   handle_fun_t fun);

void priorityFirst(Node *node, nodeCheck_t is_ok, weight_fun_t w_fun,
                   modify_fun_t fun);
